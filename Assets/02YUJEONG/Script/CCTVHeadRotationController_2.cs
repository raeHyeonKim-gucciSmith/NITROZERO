...
{
    /// <summary>
    /// CCTV / 과속 단속 카메라 2 (Wall-E 스타일) 헤드 상승 및 회전 제어 스크립트
    /// cameraheadermove_2를 중심으로 카메라얼굴_2가 회전하며 제어됩니다.
    /// 
    /// 단축키:
    /// - O 키: 기본 상태(Y: 0.0051)에서 상승 상태(Y: 0.047)로 부드럽게 '지이잉~' 쭉 올라옴 (토글)
    /// - [ 키: 오른쪽으로 고개 회전 후 유지
    /// - ] 키: 왼쪽으로 고개 회전 후 유지
    /// - P 키: 원래 정면 각도로 리셋
    /// </summary>
    public class CCTVHeadRotationController_2 : MonoBehaviour
    {
        ...
        [Tooltip("평소 내려가 있는 기본 Y 위치 (카메라얼굴_2 기본값: 0.0051)")]
        public float downY = 0.0051f;

        [Tooltip("쭉 올라왔을 때의 목표 Y 위치 (카메라얼굴_2 상승값: 0.047)")]
        public float upY = 0.047f;
        ...
        [Tooltip("회전축이 될 오브젝트 (cameraheadermove_2). 비워두면 자동으로 씬에서 cameraheadermove_2를 찾습니다.")]
        ...
        private float _currentY = 0.0051f;
        private float _targetY = 0.0051f;
        ...
        private void AutoFindReferences()
        {
            ...
            {
                if (transform.parent != null && transform.parent.name.ToLower().Contains("cameraheadermove_2"))
                {
                    pivotTransform = transform.parent;
                }
                if (pivotTransform == null)
                {
                    var child = transform.Find("cameraheadermove_2");
                    if (child != null) pivotTransform = child;
                }
                if (pivotTransform == null)
                {
                    var go = GameObject.Find("cameraheadermove_2");
                ...
                if (pivotTransform == null)
                {
                    ...
                    {
                        if (t.name.ToLower().Contains("cameraheadermove_2"))
                        {
                            pivotTransform = t;
                            break;
                        }
            ...
            if (targetHead == null)
            {
                if (transform.name.Contains("카메라얼굴_2") || transform.name.ToLower().Contains("cameraheadermove_2"))
                {
                    var childFace = transform.Find("카메라얼굴_2");
                    if (childFace != null)
                    {
        ...
        private void UpdateRotation()
        {
            ...
            // 수평 피벗 좌표를 유지하면서 현재 상승 높이(Y)에 맞춤
        ...
        /// <summary>
        /// 아래로 쏙 내려가기 (첫 번째 사진 상태: Y: 0.0051)
        /// </summary>
