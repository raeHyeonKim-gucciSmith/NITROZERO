 ...
 public override void OnInspectorGUI(){
  ...
  if(serializedObject.FindProperty("cameraPlaybackMode").enumValueIndex==1){
   ...
   if(index>=0&&index<profiles.arraySize){var setup=profiles.GetArrayElementAtIndex(index);
    ...
    if(perShot){EditorGUILayout.PropertyField(setup.FindPropertyRelative("startTime"));EditorGUILayout.PropertyField(setup.FindPropertyRelative("endTime"));}
    if(serializedObject.FindProperty("repositionForTake").boolValue)EditorGUILayout.PropertyField(setup.FindPropertyRelative("redStartWorldX"));
   }else if(perShot)EditorGUILayout.HelpBox("SHOT 설정이 없어 공통 촬영 구간을 사용합니다.",MessageType.Warning);
   if(!perShot||index<0||index>=profiles.arraySize){EditorGUILayout.PropertyField(serializedObject.FindProperty("takeStartTime"));EditorGUILayout.PropertyField(serializedObject.FindProperty("takeEndTime"));}
  ...
  if(single&&d.repositionForTake)EditorGUILayout.HelpBox("선택 SHOT의 촬영 시작 시 빨간차가 지정 X에 오도록 전체 동선을 한 번 이동합니다. 높이·차선·차량 간격·맵은 그대로입니다. 준비 재생은 맵 밖일 수 있으니 '촬영 구간 재생 중'부터 사용하세요. 번호/위치/시간 변경 후 다시 재생 버튼을 누릅니다.",MessageType.Info);
  if(single&&d.takeCameraNumber==1)EditorGUILayout.HelpBox("01은 고정 카메라입니다. 길게 촬영해도 차량이 지나간 뒤에는 빈 도로가 보일 수 있습니다.",MessageType.Info);
  ...
  EditorGUILayout.Space();serializedObject.Update();DrawPropertiesExcluding(serializedObject,"m_Script","cameraPlaybackMode","takeCameraNumber","takeStartTime","takeEndTime","loopTake","takeLoopDelay","useShotTakeTimes","repositionForTake","shotTakes");serializedObject.ApplyModifiedProperties();
