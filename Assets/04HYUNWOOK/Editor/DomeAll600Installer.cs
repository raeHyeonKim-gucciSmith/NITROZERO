...
[InitializeOnLoad]
public static class DomeAll600Installer
{
    const string Request="Library/DomeAll600.request";
    static DomeAll600Installer(){EditorApplication.delayCall+=Check;}
    static void Check()
    {
        ...
        try
        {
            ...
            seq.StopAndRestore();
            var backup="Documentation/DomeCameraPreview/BeforeAll600-"+DateTime.Now.ToString("yyyyMMdd-HHmmss");
            Directory.CreateDirectory(backup);
            if(!EditorSceneManager.SaveScene(scene,backup+"/HW_domeInTheMoon.unity",true))throw new IOException("Backup failed");
            seq.fastSpeedMetresPerSecond=600f/3.6f;seq.lastShotSpeedKmh=600;
            ...
            File.WriteAllText("Library/DomeAll600.success.txt","Saved "+scene.path+"\nBase and last-shot vehicle speeds are both 600 km/h. Auto exits Play Mode at timeline end ("+seq.PlaybackDuration+"s).\nBackup: "+backup+"\nNo Play Mode or tests.");
        }
        catch(Exception e){File.WriteAllText("Library/DomeAll600.error.txt",e.ToString());}
    }
}
