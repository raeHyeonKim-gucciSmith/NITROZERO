...
[InitializeOnLoad]
public static class DomeFasterVehiclesInstaller
{
    const string Request="Library/FasterDomeVehicles.request";
    static DomeFasterVehiclesInstaller(){EditorApplication.delayCall+=Check;}
    static void Check()
    {
        ...
        File.Delete(Request);
        try{Apply();}catch(Exception e){File.WriteAllText("Library/DomeFasterVehicles.error.txt",e.ToString());Debug.LogException(e);}
    }
    static void Apply()
    {
        ...
        string backup="Documentation/DomeCameraPreview/BeforeFasterVehicles-"+DateTime.Now.ToString("yyyyMMdd-HHmmss");
        Directory.CreateDirectory(backup);
        if(!EditorSceneManager.SaveScene(scene,backup+"/HW_domeInTheMoon.unity",true))throw new IOException("Backup failed");
        var path=AssetDatabase.GenerateUniqueAssetPath("Assets/04HYUNWOOK/Cinematics/HW_Dome_5Shots_Fast_Continue_5s.playable");
        ...
        seq.equalShotDurations=true; seq.fastSpeedMetresPerSecond=140f; seq.continueVehiclesAfterCamera=true;
        for(int i=0;i<5;i++)
        {
        ...
        File.WriteAllText("Library/DomeFasterVehicles.success.txt","Saved "+scene.path+"\nTimeline: "+path+"\nFive 1-second shots. Speed 140 m/s. Vehicles continue after 5 seconds. Car passes at 4.5 seconds.\nBackup: "+backup+"\nNo Play Mode or tests.");
    }
}
