...
public class RacingHudController : MonoBehaviour
{
    ...
    public bool MapVisible { get; private set; } = true;
    [Header("Live Minimap")]
    ...
    void Bind()
    {
        ...
        ApplyMapVisibility();
        // Respect UXML selection settings; do not disable picking on every live element.
        boundRoot.pickingMode = PickingMode.Position;
    }
    ...
    void UpdateMapToggle()
    {
#if ENABLE_INPUT_SYSTEM
        var keyboard = UnityEngine.InputSystem.Keyboard.current;
        if (keyboard != null && keyboard.mKey.wasPressedThisFrame) ToggleMap();
    ...
    {
        var map = boundRoot?.Q("minimap-panel");
        if (map != null) map.style.display = MapVisible ? StyleKeyword.Null : new StyleEnum<DisplayStyle>(DisplayStyle.None);
        boundRoot?.Query<VisualElement>(className: "map-artwork").ForEach(part =>
            part.style.display = MapVisible ? StyleKeyword.Null : new StyleEnum<DisplayStyle>(DisplayStyle.None));
    }
    ...
    public partial class DigitalReadout : VisualElement
    {
        ...
        public DigitalReadout()
        {
            ...
            if (font != null)
            {
                ...
                // Avoid the ATG glyph-atlas path seen in the editor preview OOM crash.
                glyphs.style.unityTextGenerator = new StyleEnum<TextGeneratorType>(TextGeneratorType.Standard);
                ...
                glyphs.style.overflow = Overflow.Hidden;
