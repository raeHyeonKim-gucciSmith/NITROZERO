Shader "UI/Racing HUD Curved"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            float _Curvature, _OutsideOpacity, _ShieldCoverage, _ShieldOpacity;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                fixed4 c;
                if(any(uv<0)||any(uv>1)) c=fixed4(0,0,0,_OutsideOpacity);
                else c=tex2D(_MainTex,uv);
                // RawImage UV y=1 is the top: close downwards in unwarped screen space.
                float shield = _ShieldCoverage > 0 ? step(1-_ShieldCoverage,i.uv.y)*_ShieldOpacity : 0;
                c=lerp(c,fixed4(0,0,0,1),shield);
