    ...
    private static void CreateGuide(Scene scene)
    {
        ...
        foreach (GameObject root in scene.GetRootGameObjects())
            if (root.name == "Boost Road Guide (20m Flat Corridor)" ||
                root.name == "Boost Road Guide (35m Flat Corridor)") guide = root;
        if (guide == null)
        {
            guide = new GameObject("Boost Road Guide (35m Flat Corridor)");
        ...
        guide.name = "Boost Road Guide (35m Flat Corridor)";
        guide.transform.localScale = new Vector3(Length, 1f, 35f);
